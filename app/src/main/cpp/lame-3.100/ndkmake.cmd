...
set result=%errorlevel%
if not %result%==0 goto end
if not exist obj\libs mkdir obj\libs
xcopy libs obj\libs /s /y>nul 2>nul
set result=%errorlevel%
if not %result%==0 echo copy file failed
if not %result%==0 goto end
call ndk-build NDK_PROJECT_PATH=. APP_BUILD_SCRIPT=Android.mk.executable
set result=%errorlevel%
set path=%oldpath%
if not %result%==0 goto end
...
xcopy obj\libs libs /s /y>nul 2>nul
set result=%errorlevel%
if not %result%==0 goto end
